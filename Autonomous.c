...
void moveForward(float tiles,int leftSpeed,int rightSpeed){
	...
	waitUntil(getMotorEncoder(rightBackWheel) >= 1200);
	setWheelSpeed(0);
}
...
void autonomousLauncher(){
	...
	moveForward(1.5,60,70);

	//******** MOVE BACKWARD ********
	//move backward 24 inches and stop
	moveBackward(1.5,-60,-70);
}
...
void display(){
	clearLCDLine(0);
	clearLCDLine(1);
	sprintf(distance, "%1.2f%c", getDistance(1.5));
	displayLCDCenteredString(0, distance);
	sprintf(rotation, "%1.2f%c", getMotorEncoder(rightBackWheel));
